...
test.describe('Episodes', () => {
  ...
  test('should create a minimal episode with required fields only', async ({
    ...
    const severitySlider = page.locator('[name="severity"]').first();
    await severitySlider.fill(minimalEpisode.severity.toString());
    ...
    await expect(
      page.getByText(/episode created successfully/i)
    ).toBeVisible({ timeout: 10000 });

    // Verify the episode appears in the list
    // Close dialog first (it might auto-close, but let's be sure)
    await page.keyboard.press('Escape');

    // Check that we have at least one episode card
  ...
  test('should create a complete episode with all fields', async ({ page }) => {
    ...
    await expect(page.getByText(new RegExp(completeEpisode.severity.toString()))).toBeVisible();
  });
  ...
  test('should edit an existing episode', async ({ page }) => {
    ...
    // Wait for success message
    await expect(
      page.getByText(/episode updated successfully/i)
    ).toBeVisible({ timeout: 10000 });
  ...
  test('should add and remove medications dynamically', async ({ page }) => {
    ...
    await expect(page.getByDisplayValue('Ibuprofen')).toBeVisible();
    ...
    const removeButtons = page.getByRole('button').filter({ hasText: '' }).filter({
  ...
  test('should handle episode with end time', async ({ page }) => {
    ...
    const severitySlider = page.locator('[name="severity"]').first();
    await severitySlider.fill(completeEpisode.severity.toString());
...
async function fillEpisodeForm(page: any, episodeData: TestEpisodeData) {
  ...
  // Severity
  const severitySlider = page.locator('[name="severity"]').first();
  await severitySlider.fill(episodeData.severity.toString());
  ...
  if (episodeData.symptoms && episodeData.symptoms.length > 0) {
    await page.getByRole('button', { name: /select symptoms/i }).click();
  ...
  if (episodeData.triggers && episodeData.triggers.length > 0) {
    await page.getByRole('button', { name: /select possible triggers/i }).click();
  ...
  if (episodeData.medications && episodeData.medications.length > 0) {
    for (let i = 0; i < episodeData.medications.length; i++) {
      ...
      if (medication.effectiveness) {
        ...
        await effectivenessInputs.nth(i).fill(medication.effectiveness.toString());
      }
    }
  }
